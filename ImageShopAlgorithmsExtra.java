 ...
 * 
 * Extra Features: I have added three extra filters to this file. I added
 * a blue filter which turns the image more blue, a pink filter
 * which changes the image to appear more pink, and a yellow filter to 
 * turn the image more of a yellow color.
 */
 ...
 * and equalize the picture selected. They can turn the image blue,
 * yellow, and pink with the three additional filters. It is similar 
 * to the program photoshop.
 */
...
public class ImageShopAlgorithmsExtra implements ImageShopAlgorithmsInterfaceExtra {
	...
	/*
	 * Goes to each pixel in the image selected by the user
	 * and gets the blue and green values of that pixel.
	 * It then creates a new pixel with those same blue 
	 * and green values and changes the red value to 0,
	 * creating the blue filter.
	 */
	public GImage turnBlue(GImage source) {
		int[][] pixels = source.getPixelArray();
		int width = pixels[0].length;
		int height = pixels.length;
		for(int row = 0; row < height; row++) {
			for(int col = 0; col < width; col++) { // goes through each pixel in the image selected by the user
				...
				int blue = GImage.getBlue(pixel);
				int green = GImage.getGreen(pixel);
				int transparentPixel = GImage.createRGBPixel(0, green, blue); // sets red value to 0 to enhance the green and blue colors
				pixels[row][col] = transparentPixel;
			}
		}
	...
	/*
	 * Goes to each pixel in the image selected by the user
	 * and gets the red and blue values of that pixel.
	 * It then creates a new pixel with those same red 
	 * and blue values and changes the green value to 0,
	 * creating the pink filter.
	 */
	public GImage turnPink(GImage source) {
		int[][] pixels = source.getPixelArray();
		int width = pixels[0].length;
		int height = pixels.length;
		for(int row = 0; row < height; row++) {
			for(int col = 0; col < width; col++) { // goes through each pixel in the image selected by the user
				...
				int red = GImage.getRed(pixel);
				int blue = GImage.getBlue(pixel);
				int transparentPixel = GImage.createRGBPixel(red, 0, blue); // sets green value to 0 to enhance the blue and red colors
				pixels[row][col] = transparentPixel;
			}
		}
		source.setPixelArray(pixels);
	...
	/*
	 * Goes to each pixel in the image selected by the user
	 * and gets the red and green values of that pixel.
	 * It then creates a new pixel with those same red 
	 * and green values and changes the blue value to 0,
	 * creating the yellow filter.
	 */
	public GImage turnYellow(GImage source) {
		int[][] pixels = source.getPixelArray();
		int width = pixels[0].length;
		int height = pixels.length;
		for(int row = 0; row < height; row++) {
			for(int col = 0; col < width; col++) { // goes through each pixel in the image selected by the user
				...
				int red = GImage.getRed(pixel);
				int green = GImage.getGreen(pixel);
				int transparentPixel = GImage.createRGBPixel(red, green, 0); // sets blue value to 0 to enhance the green and red colors
				pixels[row][col] = transparentPixel;
			}
		}
		source.setPixelArray(pixels);
